...
<?php while ( $row = $results->fetch_assoc() ) : ?>
	<tr>	
		<td>
			<!-- Add the track_id a a parameter so that delete.php knows exactly which song to delete.  -->
	...
	<script>
		...
		for( let i = 0; i < deleteButtons.length; i++ ) {
			deleteButtons[i].onclick = function() {
				// Show a popup here
				// return false is similar to event.preventDefault() - it prevents the deafult behavior
				// confirm() method returns TRUE or FALSE. TRUE if user clicked "Ok" FALSE if user clicked "Cancel".
